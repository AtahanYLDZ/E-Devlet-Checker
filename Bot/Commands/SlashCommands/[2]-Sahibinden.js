...
module.exports = {
    name: "sahibinden",
    commandData: new SlashCommandBuilder().setName("sahibinden").setDescription(`Sahibinden hesabi çikarir.`).addNumberOption(o => o.setName("sayi").setDescription("Kaç tane hesap çikarilacak?").setRequired(true)),
    async execute(client, int, embed) {
        ...
        const loglar = fs.readFileSync("./Checker/Sahibinden/loglar.txt", "utf-8").split("\n");
        const usedLoglar = fs.readFileSync("./Checker/Sahibinden/used.txt", "utf-8").split("\n");
        ...
        fs.writeFileSync("./Checker/Sahibinden/used.txt", hesaplar.slice(0, sayi).concat(usedLoglar).join("\n"), "utf-8");

        const txt = new AttachmentBuilder(Buffer.from(hesaplar.slice(0, sayi).join("\n"), "utf-8"), { name: `${hesaplar.slice(0, sayi).length} Sahibinden.txt` });
